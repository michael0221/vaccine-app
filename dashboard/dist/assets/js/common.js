...
function createUploader(el,uploadPath, fnSucc, jsonData, mimeType){
    ...
    var config = {
        ...
        init: function () {
            ...
            this.on('success', function (file, response) { fnSucc(file, response, this); });
...
function enrolCourse(coursecode){
    dopost({
...
function callPayPanel(data){
    var postData =  {};

    var postData =  {
        "itemname":data.item.name,  //Name of item to be paid for
        "itemcode":data.payment.itemcode, //item code generated by paypanel
        "amount":data.item.amount, //cost of this purchase
        "email":data.user.email, //customer email
        "customername":data.user.fullname, //customer fullname
        "mobile":data.user.mobile, //customer phone no
        "guid":data.user.email //customer unique identifier on your platform
    };
    var payObj = new payPanelClient({
        url : api_link + "/paypanel/pay.php",
        //alert : bootbox.alert,
        alert : function(msg){Swal.fire("Error",msg)},
        fail : function(response){
            //Display fail response
            Swal.fire("Transaction Failed","Transaction Failed. Please try again later","error");
        //     bootbox.dialog({
        ...
        success : function(response){
            //Display receipt or success response
            Swal.fire("Transaction successful",JSON.stringify(response),"success");
